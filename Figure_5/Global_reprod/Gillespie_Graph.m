function [G, dt] = Gillespie_Graph(G, G_r, params)
...
G_R = 1; %amount of global reproduction
mu = 4; %number of connections for local reproduction

%% Event probabilities
birth = G_R*b*mu*sum(S)*sum(O)/K + (1-G_R)*b*(S*(O*G_r.adjacency)');
sus_death = d*sum(S);
...
if (U <= birth/R_T)                             %birth event
    ...
    prob_vec = [0, cumsum(G_R*b*mu*sum(S)/K*O + (1-G_R)*b*(O.*(S*G_r.adjacency)))]/R_T;
    f = find(prob_vec < U);
