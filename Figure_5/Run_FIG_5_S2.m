...
load('Gt0.mat')
means(1,1) = mean(mean_alpha_vec);
stds(1,1) = std(mean_alpha_vec);

load('Gt0k02.mat')
means(2,1) = mean(mean_alpha_vec);
stds(2,1) = std(mean_alpha_vec);

%% Heterogeneity at birth results
load('Gt0.mat')
means(3,1) = mean(mean_alpha_vec);
stds(3,1) = std(mean_alpha_vec);

load('Gt0k02_birth.mat')
means(4,1) = mean(mean_alpha_vec);
stds(4,1) = std(mean_alpha_vec);

%% Infected Give Birth Results
load('Gt0_InfBirth.mat')
means(5,1) = mean(mean_alpha_vec);
stds(5,1) = std(mean_alpha_vec);

load('Gt0k02_InfBirth.mat')
means(6,1) = mean(mean_alpha_vec);
stds(6,1) = std(mean_alpha_vec);

%% Global Reproduction results
load('Gt0_GR.mat')
means(7,1) = mean(mean_alpha_vec);
stds(7,1) = std(mean_alpha_vec);

load('Gt0k02_GR.mat')
means(8,1) = mean(mean_alpha_vec);
stds(8,1) = std(mean_alpha_vec);
