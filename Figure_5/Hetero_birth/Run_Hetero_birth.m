%%% Running the irregular network evolution model where the connections /
%%% degree of nodes/individuals is determined at birth

% Exactly the same as other run codes, except in a couple of places
% 1. Gillespie_Graph now removes and adds new edges upon the birth of a new
% individuals. The removal of edges from the previous empty node can occur
% in the same event as the edges from an empty node are meaningless and
% serve no purpose
% 2. This requires a new function to add new connections:
% 'add_new_connections'
...
tic
while t < t_end
    [G, dt] = Gillespie_Graph(G, G_R, [b, d, G_T], k, mu);
    t = t + dt;
