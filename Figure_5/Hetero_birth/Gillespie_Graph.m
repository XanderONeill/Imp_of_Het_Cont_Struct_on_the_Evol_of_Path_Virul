function [G, dt] = Gillespie_Graph(G, G_r, params, k, mu)
...
if (U <= birth/R_T)                             %birth event
    ...
    prob_vec = [0, cumsum(b*(O.*(S*G_r.adjacency)))]/R_T;
    f = find(prob_vec < U);
    ind = f(end); % ind denotes the index of the empty node that will become sus
    % f = prob_vec<U;
    % ind = sum(f);

    % remove previous edge connections coming from new susceptible node
    G = rmedge(G, ind, successors(G, ind));

    % add new set of edges from new susceptible node
    new_ind = add_new_connections(G_r, ind, k, mu);
    G = addedge(G, ind, new_ind, 1);
...
elseif (U <= (birth+sus_death+inf_death)/R_T)   %infected death
    ...
    % GLOBAL TRANS - p*sum(S)/K*sum((beta.*I)'.*A2, 2)'
